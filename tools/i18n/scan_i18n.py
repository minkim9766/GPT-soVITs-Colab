...
DEFAULT_LANGUAGE: str = "zh_CN"  # ÈªòË?ËØ??
TITLE_LEN: int = 60  # ?áÈ¢ò?æÁ§∫?øÂ∫¶
KEY_LEN: int = 30  # ??êç?æÁ§∫?øÂ∫¶
SHOW_KEYS: bool = False  # ??ê¶?æÁ§∫??ø°??
SORT_KEYS: bool = False  # ??ê¶?âÂÖ®Â±Ä??êç?ôÂÖ•?á‰ª∂
...
def update_i18n_json(json_file, standard_keys):
    ...
    if len(miss_keys) > 0:
        ...
        for key in miss_keys:
            if DEFAULT_LANGUAGE in json_file:
                ...
                # ?∂‰ªñËØ???ÑÂÄºË?ÁΩ?∏∫ #! + ??êç‰ª•Ê†áÊ≥®Êú™Ë¢´ÁøªËØ?
                json_data[key] = "#!" + key
            if SHOW_KEYS:
                print(f"{'Added Missing Key'.ljust(KEY_LEN)}: {key}")
    # ËØÜÂà´Â§ö‰Ωô?ÑÈîÆÂπ∂Âà†??
    ...
    # Â∞ÜÂ§Ñ?ÜÂêé?ÑÁªì?úÂÜô??JSON ?á‰ª∂
