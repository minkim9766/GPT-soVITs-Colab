...
def load_audio(file, sr):
    try:
        ...
        file = clean_path(file)  # ?≤Ê?Â∞èÁôΩ?∑Ë∑ØÂæÑÂ§¥Â∞æÂ∏¶‰∫ÜÁ©∫?ºÂíå"?åÂõûËΩ?
        ...
        raise RuntimeError(i18n("?≥È¢ë?†ËΩΩÂ§±Ë¥•"))
        ...
        if is_train:
            for file, status in zip(file_list, files_status):
                if status:
                    ...
                    gr.Warning(file)
            gr.Warning(i18n("‰ª•‰∏ã?á‰ª∂?ñÊñá‰ª∂Â§π‰∏çÂ≠ò??))
            return False
...
def check_details(path_list=None, is_train=False, is_dataset_processing=False):
    if is_dataset_processing:
        ...
        if not list_path.endswith(".list"):
            gr.Warning(i18n("ËØ∑Â°´?•Ê?Á°?öÑListË∑?æÑ"))
            return
        if audio_path:
            if not os.path.isdir(audio_path):
                gr.Warning(i18n("ËØ∑Â°´?•Ê?Á°?öÑ?≥È¢ë?á‰ª∂Â§πË∑ØÂæ?))
                return
        ...
        if os.path.exists(wav_path):
            ...
            gr.Warning(wav_path + i18n("Ë∑?æÑ?ôË?"))
        return
    if is_train:
        ...
        with open(phone_path, "r", encoding="utf-8") as f:
            if f.read(1):
                ...
                gr.Warning(i18n("Áº∫Â∞ë?≥Á¥†?∞ÊçÆ??))
        if os.listdir(hubert_path):
            ...
        else:
            gr.Warning(i18n("Áº∫Â∞ëHubert?∞ÊçÆ??))
        if os.listdir(wav_path):
            ...
        else:
            gr.Warning(i18n("Áº∫Â∞ë?≥È¢ë?∞ÊçÆ??))
        df = pd.read_csv(semantic_path, delimiter="\t", encoding="utf-8")
        if len(df) >= 1:
            ...
        else:
            gr.Warning(i18n("Áº∫Â∞ëËØ?πâ?∞ÊçÆ??))
