...
def kill_process(pid, process_name=""):
    if system == "Windows":
        ...
        kill_proc_tree(pid)
    print(process_name + i18n("进程已终�?))


def process_info(process_name="", indicator=""):
    if indicator == "opened":
        return process_name + i18n("已�???)
    elif indicator == "open":
        return i18n("开??) + process_name
    elif indicator == "closed":
        return process_name + i18n("已关??)
    elif indicator == "close":
        return i18n("?�闭") + process_name
    elif indicator == "running":
        return process_name + i18n("运行�?)
    elif indicator == "occupy":
        return process_name + i18n("?�用�?) + "," + i18n("?�?�终止才?��???��一次任??)
    elif indicator == "finish":
        return process_name + i18n("已完??)
...
process_name_subfix = i18n("?�频?�注WebUI")
...
process_name_uvr5 = i18n("人声?�离WebUI")
...
process_name_tts = i18n("TTS?�理WebUI")
...
p_train_SoVITS = None
process_name_sovits = i18n("SoVITS�?��")
...
def switch_version(version_):
    ...
    if pretrained_sovits_name[version] != "" and pretrained_gpt_name[version] != "":
        ...
        gr.Warning(i18n("?�下载模??) + ": " + version.upper())
    set_default()
    return (
...
with gr.Blocks(title="GPT-SoVITS WebUI", analytics_enabled=False, js=js, css=css) as app:
    gr.HTML(
        top_html.format(
            i18n("?�软件以MIT?��?开�? 作者不对软件具备任何控?�力, 使用�?��?�、传??��件�??�的声音?�自负全�?")
    ...
    with gr.Tabs():
        with gr.TabItem("0-" + i18n("?�置?�据?�获?�工??)):  # ?�前?�机?�片?��?uvr5?�内�?>uvr5->slicer->asr->?�标
            with gr.Accordion(label="0a-" + i18n("UVR5人声伴奏?�离&?�混?�去延迟工具")):
                with gr.Row():
            ...
            with gr.Accordion(label="0b-" + i18n("�?��?�分工具")):
                with gr.Row():
                    with gr.Column(scale=3):
                        with gr.Row():
                            slice_inp_path = gr.Textbox(label=i18n("?�频?�动?�分输入�?��，可?�件??��件夹"), value="")
                            slice_opt_root = gr.Textbox(
                                label=i18n("?�分?�的子音频的输出?�目�?), value="output/slicer_opt"
                            )
                        with gr.Row():
                            threshold = gr.Textbox(
                                label=i18n("threshold:?�量小于这个?�视作静?�的备选切?�点"), value="-34"
                            )
                            min_length = gr.Textbox(
                                label=i18n("min_length:每�??�小多?�，如果第�?段太????�和?�面段连起来?�到超过这个??),
                                value="4000",
                            )
                            min_interval = gr.Textbox(label=i18n("min_interval:?�??��?�间??), value="300")
                            hop_size = gr.Textbox(
                                label=i18n("hop_size:?�么算音?�曲线，越小精度越大计算?�越高（不是精度越大?�果越�?�?),
                                value="10",
                            )
                            max_sil_kept = gr.Textbox(label=i18n("max_sil_kept:?�完?�静?��?多留多长"), value="500")
                        with gr.Row():
                            _max = gr.Slider(
                                minimum=0,
                                maximum=1,
                                step=0.05,
                                label=i18n("max:归�??�后?�大值多�?),
                                value=0.9,
                                interactive=True,
                            )
                            alpha = gr.Slider(
                                ...
                                maximum=1,
                                step=0.05,
                                label=i18n("alpha_mix:混多少比例归一?�后?�频进来"),
                                value=0.25,
                                interactive=True,
                            )
                        with gr.Row():
            ...
            # gr.Markdown(value="0bb-" + i18n("�?��?�噪工具")+i18n("(不稳定，?�别?�，??��?�化模型?�果�?"))
            with gr.Row(visible=False):
                with gr.Column(scale=3):
                    with gr.Row():
                        denoise_input_dir = gr.Textbox(label=i18n("输入?�件夹路�?), value="")
                        denoise_output_dir = gr.Textbox(label=i18n("输出?�件夹路�?), value="output/denoise_opt")
                    with gr.Row():
            ...
            with gr.Accordion(label="0c-" + i18n("�?��识别工具")):
                with gr.Row():
                    with gr.Column(scale=3):
                        with gr.Row():
                            asr_inp_dir = gr.Textbox(
                                label=i18n("输入?�件夹路�?), value="D:\\GPT-SoVITS\\raw\\xxx", interactive=True
                            )
                            asr_opt_dir = gr.Textbox(
                                label=i18n("输出?�件夹路�?), value="output/asr_opt", interactive=True
                            )
                        with gr.Row():
                            ...
                            asr_size = gr.Dropdown(
                                label=i18n("ASR 模型尺�?"), choices=["large"], interactive=True, value="large"
                            )
                            asr_lang = gr.Dropdown(
                                label=i18n("ASR �??设置"), choices=["zh", "yue"], interactive=True, value="zh"
                            )
                            asr_precision = gr.Dropdown(
                                label=i18n("?�据类型精度"), choices=["float32"], interactive=True, value="float32"
                            )
                        with gr.Row():
                ...
                def change_lang_choices(key):  # ?�据?�择?�模?�修?�可?�的�??
                    return {"__type__": "update", "choices": asr_dict[key]["lang"], "value": asr_dict[key]["lang"][0]}

                def change_size_choices(key):  # ?�据?�择?�模?�修?�可?�的模型尺�?
                    return {"__type__": "update", "choices": asr_dict[key]["size"], "value": asr_dict[key]["size"][-1]}

                def change_precision_choices(key):  # ?�据?�择?�模?�修?�可?�的�??
                    if key == "Faster Whisper (多�?�?":
            ...
            with gr.Accordion(label="0d-" + i18n("�?��?�本?��??�注工具")):
                with gr.Row():
                    with gr.Column(scale=3):
                        with gr.Row():
                            path_list = gr.Textbox(
                                label=i18n("?�注?�件�?�� (?�文件后缀 *.list)"),
        ...
        with gr.TabItem(i18n("1-GPT-SoVITS-TTS")):
            ...
            with gr.Accordion(label=i18n("预�?练模?�路�?), open=False):
                with gr.Row():
                    with gr.Row(equal_height=True):
            ...
            with gr.TabItem("1A-" + i18n("�?��?�格式化工具")):
                with gr.Accordion(label=i18n("输出logs/实验?�目录下应有23456开头的?�件?�文件夹")):
                    with gr.Row():
                        with gr.Row():
                            inp_text = gr.Textbox(
                                label=i18n("*?�本?�注?�件"),
                        ...
                        with gr.Row():
                            inp_wav_dir = gr.Textbox(
                                ...
                                interactive=True,
                                placeholder=i18n(
                                    "填切?�后?�频?�?�目录！读取?�音频文件完?�路�?该目�??�接-list?�件?�波形�?应的?�件?�（不是?�路径）?�如?�留空则使用.list?�件?�的绝�??�路径�?
                                ),
                                scale=10,
                            )
                    ...
                    with gr.Row():
                        with gr.Row():
                            gpu_numbers1a = gr.Textbox(
                                label=i18n("GPU?�号�??�割，每个卡?��?个进�?),
                                value="%s-%s" % (gpus, gpus),
                                interactive=True,
                            )
                        with gr.Row():
                            bert_pretrained_dir = gr.Textbox(
                                label=i18n("预�?练中?�BERT模型�?��"),
            ...
            with gr.TabItem("1B-" + i18n("�?���?��")):
                with gr.Accordion(label="1Ba-" + i18n("SoVITS �?��: 模型?�重?�件??SoVITS_weights/")):
                    with gr.Row():
                        with gr.Column():
                            with gr.Row():
                                ...
                                total_epoch = gr.Slider(
                                    ...
                                    step=1,
                                    label=i18n("?��?练轮?�total_epoch，不建�?太高"),
                        ...
                        with gr.Column():
                            with gr.Column():
                                if_save_latest = gr.Checkbox(
                                    label=i18n("??��仅保存�??�的?�重?�件以节?�硬?�空??),
                                    value=True,
                                    interactive=True,
                                    show_label=True,
                                )
                                if_save_every_weights = gr.Checkbox(
                                    label=i18n("??��?�每次保存时?�点将�?终小模型保存?�weights?�件�?),
                                    value=True,
                                    interactive=True,
                                    show_label=True,
                                )
                ...
                with gr.Accordion(label="1Bb-" + i18n("GPT �?��: 模型?�重?�件??GPT_weights/")):
                    with gr.Row():
                        with gr.Column():
                            with gr.Row():
            ...
            with gr.TabItem("1C-" + i18n("?�理")):
                gr.Markdown(
                    value=i18n(
                        "?�择�?��完存?�在SoVITS_weights?�GPT_weights下的模型?�默认的?�个??��模，体验5秒Zero Shot TTS不�?练推?�用??
                    )
                )
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        with gr.Row():
                            gpu_number_1C = gr.Textbox(
                                label=i18n("GPU?�号,?�能�?个整??), value=gpus, interactive=True
                            )
                            refresh_button = gr.Button(i18n("?�新模型�?��"), variant="primary")
                    refresh_button.click(fn=change_choices, inputs=[], outputs=[SoVITS_dropdown, GPT_dropdown])
                with gr.Row(equal_height=True):
                    with gr.Row():
                        batched_infer_enabled = gr.Checkbox(
                            label=i18n("??��并行?�理?�本"), value=False, interactive=True, show_label=True
                        )
                        open_tts = gr.Button(
        ...
        with gr.TabItem(i18n("2-GPT-SoVITS-?�声")):
            gr.Markdown(value=i18n("?�工�?��请静?�佳??))
