...
## ?§è¡Œ?‚æ•°:
    `-a` - `ç»‘å®š?°å?, é»˜è?"127.0.0.1"`
    `-p` - `ç»‘å®šç«?£, é»˜è?9880`
    `-c` - `TTS?ç½®?‡ä»¶è·?¾„, é»˜è?"GPT_SoVITS/configs/tts_infer.yaml"`
...
GET:
```
http://127.0.0.1:9880/tts?text=?ˆå¸?›ä¸š?ªåŠ?Œä¸­?“å´©æ®‚ï¼Œä»Šå¤©ä¸‹ä¸‰?†ï¼Œ?Šå·?²å¼Šï¼Œæ?è¯šå±?¥å­˜äº¡ä¹‹ç§‹ä¹Ÿ??text_lang=zh&ref_audio_path=archive_jingyuan_1.wav&prompt_lang=zh&prompt_text=?‘æ˜¯?Œç½—æµ?€äº‘éª‘å°†?›æ™¯?ƒã€‚ä¸å¿…æ‹˜è°¨ï¼Œ?Œå°†?›ã€åª????¶çš„èº«ä»½ï¼Œä½ ç§°å‘¼?‘æ™¯?ƒä¾¿??text_split_method=cut5&batch_size=1&media_type=wav&streaming_mode=true
```
...
RESP:
?åŠŸ: ?´æ¥è¿”å› wav ?³é¢‘æµï¼Œ http code 200
å¤±è´¥: è¿”å›?…å«?™è?ä¿¡æ¯??json, http code 400
...
parser.add_argument("-c", "--tts_config", type=str, default="GPT_SoVITS/configs/tts_infer.yaml", help="tts_inferè·?¾„")
...
#     try:
#         # æ£€?¥æ–‡ä»¶ç±»?‹ï¼Œç¡?¿??Ÿ³é¢‘æ–‡ä»?
#         if not audio_file.content_type.startswith("audio/"):
#             return JSONResponse(status_code=400, content={"message": "file type is not supported"})

#         os.makedirs("uploaded_audio", exist_ok=True)
#         save_path = os.path.join("uploaded_audio", audio_file.filename)
#         # ä¿å­˜?³é¢‘?‡ä»¶?°æœ?¡å™¨ä¸Šçš„ä¸€ä¸ªç›®å½?
...
if __name__ == "__main__":
    try:
        if host == "None":  # ?¨è°ƒ?¨æ—¶ä½¿ç”¨ -a None ?‚æ•°ï¼Œå¯ä»¥è?api?‘å¬?Œæ ˆ
            host = None
