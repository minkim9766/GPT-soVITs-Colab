...
pretrained_sovits_name = {
    ...
    "v3": "GPT_SoVITS/pretrained_models/s2Gv3.pth",  ###v3v4还要检?�vocoder，算了。。�?
...
name2sovits_path = {
    # i18n("不�?练直?�推v1底模�?): "GPT_SoVITS/pretrained_models/s2G488k.pth",
    i18n("不�?练直?�推v2底模�?): "GPT_SoVITS/pretrained_models/gsv-v2final-pretrained/s2G2333k.pth",
    # i18n("不�?练直?�推v3底模�?): "GPT_SoVITS/pretrained_models/s2Gv3.pth",
    # i18n("不�?练直?�推v4底模�?): "GPT_SoVITS/pretrained_models/gsv-v4-pretrained/s2Gv4.pth",
    i18n("不�?练直?�推v2Pro底模�?): "GPT_SoVITS/pretrained_models/v2Pro/s2Gv2Pro.pth",
    i18n("不�?练直?�推v2ProPlus底模�?): "GPT_SoVITS/pretrained_models/v2Pro/s2Gv2ProPlus.pth",
}
name2gpt_path = {
    # i18n("不�?练直?�推v1底模�?):"GPT_SoVITS/pretrained_models/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt",
    i18n(
        "不�?练直?�推v2底模�?
    ): "GPT_SoVITS/pretrained_models/gsv-v2final-pretrained/s1bert25hz-5kh-longer-epoch=12-step=369668.ckpt",
    i18n("不�?练直?�推v3底模�?): "GPT_SoVITS/pretrained_models/s1v3.ckpt",
}
SoVITS_weight_root = [
    "SoVITS_weights",
    "SoVITS_weights_v2",
...
def custom_sort_key(s):
    # 使用正则表达式提?�字符串�?��?�字?�分?�非?�字?�分
    parts = re.split("(\d+)", s)
    # 将数字部?�转?�为?�数，非?�字?�分保持不变
...
CPU_INFO: str = "0\tCPU " + i18n("CPU�?��,较慢")
