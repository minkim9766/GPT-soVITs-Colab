...
# @torch.jit.script ## ‰ΩøÁî®?ÑËØùÈ¶ñÊ¨°?®ÁêÜ‰ºöÈùûÂ∏∏ÊÖ¢ÔºåËÄå‰∏î?®ÁêÜ?üÂ∫¶‰∏çÁ®≥ÂÆ?
...
class Text2SemanticDecoder(nn.Module):
    ...
    def forward(self, x, x_lens, y, y_lens, bert_feature):
        ...
        # loss
        # from feiteng: ÊØèÊ¨° duration Ë∂äÂ§ö, Ê¢?∫¶?¥Êñ∞‰πüÂ∫îËØ•Êõ¥Â§? ?Ä‰ª•Áî® sum
    ...
    # ?ÄË¶ÅÁúã‰∏ãËøô‰∏™ÂáΩ?∞Âíå forward ?ÑÂå∫?´‰ª•?äÊ≤°??semantic ?ÑÊó∂??prompts ËæìÂÖ•‰ªÄ‰π?
    def infer(
        self,
        x,
        x_lens,
        ...
        for _ in tqdm(range(1500)):
            y_emb = self.ar_audio_embedding(y)
            y_pos = self.ar_audio_position(y_emb)
            # x ?åÈÄêÊ∏êÂ¢ûÈïø??y ‰∏ÄËµ∑Ëæì?•ÁªôÊ®°Âûã
            xy_pos = torch.concat([x, y_pos], dim=1)
            y_len = y.shape[1]
            ...
            if stop:
                ...
                break
            # ?¨Ê¨°?üÊàê??semantic_ids ?å‰πã?çÁöÑ y ?ÑÊàê?∞ÁöÑ y
            # print(samples.shape)#[1,1]#Á¨¨‰?‰∏??Øbs
            # import os
            # os._exit(2333)
    ...
    def infer_panel_batch_infer(
        self,
        x: List[torch.LongTensor],  #####?®ÈÉ®?áÊú¨token
        x_lens: torch.LongTensor,
        prompts: torch.LongTensor,  ####?ÇËÄÉÈü≥È¢ëtoken
        ...
        **kwargs,
        ...
        y_mask = F.pad(  ###yy?ÑÂè≥‰∏??©Â±ï?∞Â∑¶Ëæπxy??,(y,x+y)
        ...
        ### ‰∏äÈù¢??îôËØ?öÑÔºå‰ºöÂØºËá¥padding?ÑtokenË¢??ãËßÅ"

        # Ê≠£Á°Æ?Ñpadding_maskÂ∫îË???ºö
        # |   pad_len   |  x_len  |  y_len  |
        # [[PAD, PAD, PAD, 1, 2, 3, 4, 5, 6],
        # [PAD, PAD, PAD, 1, 2, 3, 4, 5, 6],
        # [PAD, PAD, PAD, 1, 2, 3, 4, 5, 6],  ??Ë°åÊåâ?ÜË?‰πüÂ∫îËØ•Ë¢´mask?âÔºå‰ΩÜÊòØ‰∏∫‰∫Ü?≤Ê?ËÆ°ÁÆóattention?∂‰∏ç?∫Áé∞nanÔºåËøò??øù?ô‰∫ÜÔºå‰∏çÂΩ±ÂìçÁªìÊûú
        # [PAD, PAD, PAD, 1, 2, 3, 4, 5, 6],
        # [PAD, PAD, PAD, 1, 2, 3, 4, 5, 6],
        # [PAD, PAD, PAD, 1, 2, 3, 4, 5, 6],
        # [PAD, PAD, PAD, 1, 2, 3, 4, 5, 6],
        ...
        # Ê≠£Á°Æ?Ñattn_maskÂ∫îË???øô?∑ÁöÑÔº?
        # |   pad_len   |  x_len  |  y_len  |
        # [[PAD, PAD, PAD, 1, 2, 3, EOS, EOS, EOS],
        # [PAD, PAD, PAD, 1, 2, 3, EOS, EOS, EOS],
        # [PAD, PAD, PAD, 1, 2, 3, EOS, EOS, EOS],  ??Ë°åÊåâ?ÜË?‰πüÂ∫îËØ•Ë¢´mask?âÔºå‰ΩÜÊòØ‰∏∫‰∫Ü?≤Ê?ËÆ°ÁÆóattention?∂‰∏ç?∫Áé∞nanÔºåËøò??øù?ô‰∫ÜÔºå‰∏çÂΩ±ÂìçÁªìÊûú
        # [PAD, PAD, PAD, 1, 2, 3, EOS, EOS, EOS],
        # [PAD, PAD, PAD, 1, 2, 3, EOS, EOS, EOS],
        # [PAD, PAD, PAD, 1, 2, 3, EOS, EOS, EOS],
        ...
        for idx in tqdm(range(1500)):
            ...
            if idx < 11:  ###?≥Â∞ëÈ¢ÑÊµã??0‰∏™token‰∏çÁÑ∂‰∏çÁªô?úÊ?Ôº?.4sÔº?
                logits = logits[:, :-1] 
            ...
            ####### ÁßªÈô§batch‰∏?∑≤ÁªèÁîü?êÂÆåÊØïÁöÑÂ∫èÂàó,Ëøõ‰?Ê≠•‰ºò?ñË?ÁÆóÈáè
            tokens = torch.argmax(logits, dim=-1)
            reserved_idx_of_batch_for_y = None
            if (self.EOS in samples[:, 0]) or (self.EOS in tokens):  ###Â¶ÇÊûú?üÊàê?∞EOSÔºåÂàô?úÊ?
        ...
        if None in idx_list:
            for i in range(x.shape[0]):
                if idx_list[i] is None:
                    idx_list[i] = 1500 - 1  ###Â¶ÇÊûúÊ≤°Êúâ?üÊàê?∞EOSÔºåÂ∞±?®Ê?Â§ßÈïøÂ∫¶‰ª£??
    ...
    def infer_panel_naive(
        self,
        x: torch.LongTensor,  #####?®ÈÉ®?áÊú¨token
        x_lens: torch.LongTensor,
        prompts: torch.LongTensor,  ####?ÇËÄÉÈü≥È¢ëtoken
        ...
        **kwargs,
        ...
        x_attn_mask_pad = F.pad(
            x_attn_mask,
            (0, y_len),  ###xx?ÑÁ∫Ø0?©Â±ï?∞xxÁ∫?+xyÁ∫?Ôº?x,x+y)
            value=True,
        )
        y_attn_mask = F.pad(  ###yy?ÑÂè≥‰∏??©Â±ï?∞Â∑¶Ëæπxy??,(y,x+y)
            torch.triu(torch.ones(y_len, y_len, dtype=torch.bool), diagonal=1),
            (x_len, 0),
            value=False,
        )
        ...
        for idx in tqdm(range(1500)):
            ...
            if streaming_mode and (mute_emb_sim_matrix is not None) and (token_counter >= chunk_length+check_token_num):
                ...
                score[score<0]=-1
                score[:-1]=score[:-1]+score[1:] ##?ÉËôëËøûÁª≠‰∏§‰∏™token
