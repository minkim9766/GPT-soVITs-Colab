...
class Text2SemanticDataset(Dataset):
    ...
    def init_batch(self):
        ...
        for i in range(semantic_data_len):
            ...
            # (T), ??��?�要变??(1, T) -> 不�?要，?�为?�要求 len
            # 过滤?�太?�的?�本
            if (
                len(semantic_ids) > self.max_sec * self.hz
            ):  #########1###?�据token个数?�测?�时?�过滤时??0s（config?�）#40*25=1k
                num_deleted_bigger += 1
                continue
            # (T, ), 这个?�度不会很慢，�?以可以在一开始就处理，无?�??__getitem__ ?�面?�个处理####
            ...
            try:
                ...
                num_not_in += 1
                continue
            # if len(phoneme_ids) >400:###########2：改为恒定限?�为semantic/2.5就行
            if len(phoneme_ids) > self.max_sec * self.hz / 2.5:  ###########2：改为恒定限?�为semantic/2.5就行
                num_deleted_ps += 1
                continue
        ...
        min_num = 100  # 20?�接不补#30补了也不存ckpt
        ...
        if num_deleted_ps > 0:
            # 4702 for LibriTTS, LirbriTTS ??��注数?? ??��?�要筛�?> ?�要，?�值为 100 ?�极�?�?
            print(
