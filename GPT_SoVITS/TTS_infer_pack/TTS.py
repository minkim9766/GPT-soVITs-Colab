...
class TTS_Config:
    ...
    # "yue",#?‰ç²¤?±æ··?ˆè¯†??###ä¸å˜
    # "ko",#?‰éŸ©?±æ··?ˆè¯†??###ä¸å˜
    # "auto",#å¤šè?ç§å¯?¨åˆ‡?†è¯†?«è?ç§?
    # "auto_yue",#å¤šè?ç§å¯?¨åˆ‡?†è¯†?«è?ç§?
...
class TTS:
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        if if_lora_v3 == True and os.path.exists(path_sovits) == False:
            info = path_sovits + i18n("SoVITS %s åº•æ¨¡ç¼ºå¤±ï¼Œæ— æ³•åŠ è½½ç›¸åº?LoRA ?ƒé‡" % model_version)
    ...
    def init_sr_model(self):
        ...
        try:
            ...
            print(i18n("ä½ æ²¡?‰ä¸‹è½½è¶…?†æ¨¡?‹çš„?‚æ•°ï¼Œå› æ­¤ä¸è¿›è¡Œè¶…åˆ†?‚å¦‚?³è¶…?†è??ˆå‚?§æ•™ç¨‹æŠŠ?‡ä»¶ä¸‹è½½å¥?))
    ...
    def to_batch(
        ...
        for batch_idx, index_list in enumerate(batch_index_list):
            ...
            #### ?´æ¥å¯¹phones?Œbert_featuresè¿›è¡Œpad?‚ï¼ˆpaddingç­–ç•¥ä¼šå½±?T2Sæ¨¡å‹?Ÿæˆ?„ç»“?œï¼Œä½†ä¸?´æ¥å½±å“å¤è?æ¦‚ç‡?‚å½±?å¤è¯»æ¦‚?‡çš„ä¸»è¦? ç´ ?¯mask?„ç­–?¥ï¼‰
            ...
            # #### ?ˆå?phonesè¿›è¡Œembedding?å?bert_featuresè¿›è¡Œprojectï¼Œå†pad?°ç›¸?Œé•¿åº?¼Œï¼ˆpaddingç­–ç•¥ä¼šå½±?T2Sæ¨¡å‹?Ÿæˆ?„ç»“?œï¼Œä½†ä¸?´æ¥å½±å“å¤è?æ¦‚ç‡?‚å½±?å¤è¯»æ¦‚?‡çš„ä¸»è¦? ç´ ?¯mask?„ç­–?¥ï¼‰
    ...
    def run(self, inputs: dict):
        ...
        chunk_split_thershold = 0.0 # è¯¥å€¼ä»£è¡¨è?ä¹‰tokenä¸mute token?„ä½™å¼?›¸ä¼¼åº¦?ˆå€¼ï¼Œ?¥å¤§äºè??ˆå€¼ï¼Œ?™è§†ä¸ºå¯?‡åˆ†?¹ã€?

        if parallel_infer and not streaming_mode:
            print(i18n("å¹¶è¡Œ?¨ç†æ¨¡å¼å·²å???))
            ...
            print(i18n("ä¸æ”¯?åŒ?¶å???¹¶è¡Œæ¨?†å’Œæµå¼?¨ç†æ¨¡å¼ï¼Œå·²?ªåŠ¨?³é—­å¹¶è¡Œ?¨ç†æ¨¡å¼"))
            parallel_infer = False
            self.t2s_model.model.infer_panel = self.t2s_model.model.infer_panel_naive
        else:
            print(i18n("?´ç´ ?¨ç†æ¨¡å¼å·²å???))
            self.t2s_model.model.infer_panel = self.t2s_model.model.infer_panel_naive_batched

        if return_fragment and streaming_mode:
            print(i18n("æµå¼?¨ç†æ¨¡å¼ä¸æ”¯?åˆ†æ®µè¿”?ï¼Œå·²è‡ª?¨å…³??ˆ†æ®µè¿”??))
            return_fragment = False

        if (return_fragment or streaming_mode) and split_bucket:
            print(i18n("?†æ?è¿”å›æ¨¡å¼/æµå¼?¨ç†æ¨¡å¼ä¸æ”¯?åˆ†æ¡¶å¤„?†ï¼Œå·²è‡ª?¨å…³??ˆ†æ¡¶å¤„??))
            split_bucket = False


        if split_bucket and speed_factor == 1.0 and not (self.configs.use_vocoder and parallel_infer):
            print(i18n("?†æ¡¶å¤„ç†æ¨¡å¼å·²å???))
        elif speed_factor != 1.0:
            print(i18n("è¯?€Ÿè°ƒ?‚ä¸??Œ?†æ¡¶å¤„ç†ï¼Œå·²?ªåŠ¨?³é—­?†æ¡¶å¤„ç†"))
            split_bucket = False
        elif self.configs.use_vocoder and parallel_infer:
            print(i18n("å½“å???¹¶è¡Œæ¨?†æ¨¡å¼æ—¶ï¼ŒSoVits V3/4æ¨¡å‹ä¸æ”¯?åˆ†æ¡¶å¤„?†ï¼Œå·²è‡ª?¨å…³??ˆ†æ¡¶å¤„??))
            split_bucket = False
        else:
            print(i18n("?†æ¡¶å¤„ç†æ¨¡å¼å·²å…³??))
        ...
        #     print(i18n("?†æ??´éš”è¿‡å°ï¼Œå·²?ªåŠ¨è®¾ç½®ä¸?.01"))
        ...
        if not (len(list(paths)) == len(aux_ref_audio_paths) == len(self.prompt_cache["aux_ref_audio_paths"])):
            ...
            for path in aux_ref_audio_paths:
                ...
                if not os.path.exists(path):
                    print(i18n("?³é¢‘?‡ä»¶ä¸å­˜?¨ï¼Œè·³è¿‡ï¼?), path)
                    continue
        ...
        try:
            ...
            for item in data:
                ...
                print(i18n("?ç«¯å¤„ç†?çš„?‡æœ¬(æ¯å¥):"), norm_text)
                if no_prompt_text:
                    prompt = None
                else:
                    prompt = (
                ...
                if not streaming_mode:
                    ...
                    if not self.configs.use_vocoder:
                        if speed_factor == 1.0:
                            print(f"{i18n('å¹¶è¡Œ?ˆæˆä¸?)}...")
                            # ## vitså¹¶è¡Œ?¨ç† method 2
                            pred_semantic_list = [item[-idx:] for item, idx in zip(pred_semantic_list, idx_list)]
                            ...
                            for i, idx in enumerate(tqdm(idx_list)):
                                ...
                                batch_audio_fragment.append(audio_fragment)  ###è¯•è¯•?å»ºä¸å¸¦ä¸Šprompt?¨åˆ†
                    else:
                        if parallel_infer:
                            print(f"{i18n('å¹¶è¡Œ?ˆæˆä¸?)}...")
                    ...
                    for semantic_tokens, is_final in semantic_token_generator:
                        ...
                        if not self.configs.use_vocoder:
                            ...
                            raise RuntimeError(i18n("SoVits V3/4æ¨¡å‹ä¸æ”¯?æµå¼æ¨?†æ¨¡å¼?))
            ...
            traceback.print_exc()
            # å¿…é¡»è¿”å›ä¸€ä¸ªç©º?³é¢‘, ??ˆ™ä¼šå??´æ˜¾å­˜ä¸?Šæ”¾??
            yield 16000, np.zeros(int(16000), dtype=np.int16)
            # ?ç½®æ¨¡å‹, ??ˆ™ä¼šå??´æ˜¾å­˜é‡Š?¾ä¸å®Œå…¨??
            del self.t2s_model
            del self.vits_model
            self.t2s_model = None
    ...
    def empty_cache(self):
        try:
            gc.collect()  # è§?‘gc?„åƒ?¾å›?¶ã€‚é¿?å†…å­˜ä??´å¢?¿ã€?
    ...
    def audio_postprocess(
        ...
        for i, batch in enumerate(audio):
            for j, audio_fragment in enumerate(batch):
                max_audio = torch.abs(audio_fragment).max()  # ç®€?•é˜²æ­?6bit?†éŸ³
                if max_audio > 1:
