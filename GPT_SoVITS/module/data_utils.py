...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, hparams, version=None, val=False):
        ...
        names4 = set([name[:-3] for name in list(os.listdir(self.path4))])  # ?ªÈô§.pt?éÁ?
        names5 = set(os.listdir(self.path5))
        if self.is_v2Pro:
            names6 = set([name[:-3] for name in list(os.listdir(self.path7))])  # ?ªÈô§.pt?éÁ?
        self.phoneme_data = {}
        ...
        assert len(audiopaths_sid_text_new) > 1  # ?≥Â∞ë?ΩÂáëÂ§übatch sizeÔºåËøô?åtodo
    ...
    def get_audio(self, filename):
        audio_array = load_audio(filename, self.sampling_rate)  # load_audio?ÑÊñπÊ≥ïÊòØÂ∑≤ÁªèÂΩí‰??ñÂà∞-1~1‰πãÈó¥?ÑÔºå‰∏çÁî®??32768
...
class TextAudioSpeakerLoaderV3(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        audio_array24 = load_audio(
            filename, 24000
        )  # load_audio?ÑÊñπÊ≥ïÊòØÂ∑≤ÁªèÂΩí‰??ñÂà∞-1~1‰πãÈó¥?ÑÔºå‰∏çÁî®??32768######ËøôÈáå??ª•?®GPU?çÈáá?∑Âä†??
