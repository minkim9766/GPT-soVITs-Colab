...
# print(version)###GPT version?Œæ²¡?‰s2?„v2pro
...
def change_sovits_weights(sovits_path, prompt_language=None, text_language=None):
    ...
    # print("sovits?ˆæœ¬:",hps.model.version)
...
def audio_sr(audio, sr):
    ...
    if sr_model == None:
        ...
        try:
            ...
            gr.Warning(i18n("ä½ æ²¡?‰ä¸‹è½½è¶…?†æ¨¡?‹çš„?‚æ•°ï¼Œå› æ­¤ä¸è¿›è¡Œè¶…åˆ†?‚å¦‚?³è¶…?†è??ˆå‚?§æ•™ç¨‹æŠŠ?‡ä»¶ä¸‹è½½å¥?))
            return audio.cpu().detach().numpy(), sr
    return sr_model(audio, sr)


##ref_wav_path+prompt_text+prompt_language+text(?•ä¸ª)+text_language+top_k+top_p+temperature
# cache_tokens={}#?‚æœªå®ç°æ¸…ç†?ºåˆ¶
cache = {}
...
def get_tts_wav(
    ...
    if ref_wav_path:
        ...
        gr.Warning(i18n("è¯·ä¸Šä¼ å‚?ƒéŸ³é¢?))
    if text:
        pass
    else:
        gr.Warning(i18n("è¯·å¡«?¥æ¨?†æ–‡??))
    t = []
    if prompt_text is None or len(prompt_text) == 0:
        ref_free = True
    if model_version in v3v4set:
        ref_free = False  # s2v3?‚ä¸??Œref_free
    else:
        if_sr = False
    ...
    if not ref_free:
        ...
        print(i18n("å®é™…è¾“å…¥?„å‚?ƒæ–‡??"), prompt_text)
    text = text.strip("\n")
    # if (text[0] not in splits and len(get_first(text)) < 4): text = "?? + text if text_language != "en" else "." + text

    print(i18n("å®é™…è¾“å…¥?„ç›®?‡æ–‡??"), text)
    zero_wav = np.zeros(
    ...
    if not ref_free:
        with torch.no_grad():
            ...
            if wav16k.shape[0] > 160000 or wav16k.shape[0] < 48000:
                gr.Warning(i18n("?‚è€ƒéŸ³é¢‘åœ¨3~10ç§’èŒƒ?´å¤–ï¼Œè??´æ¢ï¼?))
                raise OSError(i18n("?‚è€ƒéŸ³é¢‘åœ¨3~10ç§’èŒƒ?´å¤–ï¼Œè??´æ¢ï¼?))
    ...
    if how_to_cut == i18n("?‘å››?¥ä???):
        text = cut1(text)
    elif how_to_cut == i18n("??0å­—ä???):
        text = cut2(text)
    elif how_to_cut == i18n("?‰ä¸­?‡å¥?·ã€‚åˆ‡"):
        text = cut3(text)
    elif how_to_cut == i18n("?‰è‹±?‡å¥????):
        text = cut4(text)
    elif how_to_cut == i18n("?‰æ ‡?¹ç¬¦?·åˆ‡"):
        text = cut5(text)
    while "\n\n" in text:
        text = text.replace("\n\n", "\n")
    print(i18n("å®é™…è¾“å…¥?„ç›®?‡æ–‡???‡å¥??:"), text)
    texts = text.split("\n")
    ...
    for i_text, text in enumerate(texts):
        # è§£å†³è¾“å…¥?? ‡?‡æœ¬?„ç©ºè¡Œå??´æŠ¥?™çš„??¢˜
        if len(text.strip()) == 0:
            continue
        if text[-1] not in splits:
            text += "?? if text_language != "en" else "."
        print(i18n("å®é™…è¾“å…¥?„ç›®?‡æ–‡??æ¯å¥):"), text)
        phones2, bert2, norm_text2 = get_phones_and_bert(text, text_language, version)
        print(i18n("?ç«¯å¤„ç†?çš„?‡æœ¬(æ¯å¥):"), norm_text2)
        if not ref_free:
        ...
        if model_version not in v3v4set:
            ...
            if inp_refs:
                for path in inp_refs:
                    try:  #####è¿™é‡Œ? ä¸Š?å–sv?„é€»è¾‘ï¼Œè¦ä¹ˆä??†svä¸€?†referï¼Œè¦ä¹ˆå•ä¸ªsv?•ä¸ªrefer
        ...
        max_audio = torch.abs(audio).max()  # ç®€?•é˜²æ­?6bit?†éŸ³
        if max_audio > 1:
...
def split(todo_text):
    todo_text = todo_text.replace("?¦â€?, "??).replace("?”â€?, "ï¼?)
    ...
    while 1:
        if i_split_head >= len_text:
            break  # ç»“å°¾ä¸€å®šæœ‰?‡ç‚¹ï¼Œæ?ä»¥ç›´?¥è·³?ºå³??¼Œ?€?ä?æ®µåœ¨ä¸Šæ¬¡å·²åŠ ??
...
def cut2(inp):
    ...
    if tmp_str != "":
        opts.append(tmp_str)
    # print(opts)
    if len(opts) > 1 and len(opts[-1]) < 50:  ##å¦‚æœ?€?ä?ä¸ªå¤ª??º†ï¼Œå’Œ?ä?ä¸ªåˆä¸€èµ?
...
def process_text(texts):
    _text = []
    if all(text in [None, " ", "\n", ""] for text in texts):
        raise ValueError(i18n("è¯·è¾“?¥æœ‰?ˆæ–‡??))
    for text in texts:
        if text in [None, " ", ""]:
            pass
        else:
...
with gr.Blocks(title="GPT-SoVITS WebUI", analytics_enabled=False, js=js, css=css) as app:
    ...
    with gr.Group():
        gr.Markdown(html_center(i18n("æ¨¡å‹?‡æ¢"), "h3"))
        with gr.Row():
            ...
            refresh_button = gr.Button(i18n("?·æ–°æ¨¡å‹è·?¾„"), variant="primary", scale=14)
            refresh_button.click(fn=change_choices, inputs=[], outputs=[SoVITS_dropdown, GPT_dropdown])
        gr.Markdown(html_center(i18n("*è¯·ä¸Šä¼ å¹¶å¡«å†™?‚è€ƒä¿¡??), "h3"))
        with gr.Row():
            inp_ref = gr.Audio(label=i18n("è¯·ä¸Šä¼?~10ç§’å†…?‚è€ƒéŸ³é¢‘ï¼Œè¶…è¿‡ä¼šæŠ¥?™ï¼"), type="filepath", scale=13)
            with gr.Column(scale=13):
                ref_text_free = gr.Checkbox(
                    label=i18n("å¼€??— ?‚è€ƒæ–‡?¬æ¨¡å¼ã€‚ä¸å¡«å‚?ƒæ–‡?¬äº¦?¸å½“äºå???€?)
                    + i18n("v3?‚ä¸??Œè¯¥æ¨¡å¼ï¼Œä½¿ç”¨äº†ä¼š?¥é”™??),
                    value=False,
                ...
                prompt_text = gr.Textbox(label=i18n("?‚è€ƒéŸ³é¢‘çš„?‡æœ¬"), value="", lines=5, max_lines=5, scale=1)
            with gr.Column(scale=14):
                prompt_language = gr.Dropdown(
                    label=i18n("?‚è€ƒéŸ³é¢‘çš„è¯?§"),
                ...
                inp_refs = (
                    gr.File(
                        label=i18n(
                            "??€‰é¡¹ï¼šé€šè¿‡?–æ‹½å¤šä¸ª?‡ä»¶ä¸Šä¼ å¤šä¸ª?‚è€ƒéŸ³é¢‘ï¼ˆå»ºè??Œæ€§ï¼‰ï¼Œå¹³?‡è?ˆä»–ä»¬çš„?³è‰²?‚å¦‚ä¸å¡«?™æ?é¡¹ï¼Œ?³è‰²?±å·¦ä¾§å•ä¸ªå‚?ƒéŸ³é¢‘æ§?¶ã€‚å¦‚??¾®è°ƒæ¨¡?‹ï¼Œå»ºè??‚è€ƒéŸ³é¢‘å…¨?¨åœ¨å¾?°ƒè®?»ƒ?†éŸ³?²å†…ï¼Œåº•æ¨¡ä¸?¨ç???
                        ),
                        file_count="multiple",
                    )
                    if model_version not in v3v4set
                ...
                sample_steps = (
                    gr.Radio(
                        label=i18n("?‡æ ·æ­¥æ•°,å¦‚æœè§‰å¾—???é«˜è¯•è¯•,å¦‚æœè§‰å¾—???ä½è¯•è¯•"),
                        value=32 if model_version == "v3" else 8,
                        choices=[4, 8, 16, 32, 64, 128] if model_version == "v3" else [4, 8, 16, 32],
                        visible=True,
                    )
                ...
                if_sr_Checkbox = gr.Checkbox(
                    label=i18n("v3è¾“å‡ºå¦‚æœè§‰å¾—?·å¯ä»¥è¯•è¯•å?è¶…åˆ†"),
                    value=False,
                    interactive=True,
                    show_label=True,
                    visible=False if model_version != "v3" else True,
                )
        gr.Markdown(html_center(i18n("*è¯·å¡«?™é?è¦åˆ?çš„?? ‡?‡æœ¬?Œè?ç§æ¨¡å¼?), "h3"))
        with gr.Row():
            with gr.Column(scale=13):
                text = gr.Textbox(label=i18n("?€è¦åˆ?çš„?‡æœ¬"), value="", lines=26, max_lines=26)
            with gr.Column(scale=7):
                text_language = gr.Dropdown(
                    label=i18n("?€è¦åˆ?çš„è¯?§") + i18n(".?åˆ¶?ƒå›´è¶Šå°?¤åˆ«?ˆæœè¶Šå???),
                    choices=list(dict_language.keys()),
                    value=i18n("ä¸?–‡"),
                    scale=1,
                )
                ...
                gr.Markdown(value=html_center(i18n("è¯?€Ÿè°ƒ?´ï¼Œé«˜ä¸º?´å¿«")))
                if_freeze = gr.Checkbox(
                    label=i18n("??¦?´æ¥å¯¹ä¸Šæ¬¡åˆ?ç»“?œè°ƒ?´è??Ÿå’Œ?³è‰²?‚é˜²æ­?š?ºæ€§ã€?),
                    value=False,
                    interactive=True,
                    show_label=True,
                    scale=1,
                ...
                with gr.Row():
                    speed = gr.Slider(
                        minimum=0.6, maximum=1.65, step=0.05, label=i18n("è¯?€?), value=1, interactive=True, scale=1
                    )
                ...
                gr.Markdown(html_center(i18n("GPT?‡æ ·?‚æ•°(? å‚?ƒæ–‡?¬æ—¶ä¸è¦å¤ªä½?‚ä¸?‚å°±?¨é»˜è®?ï¼?)))
                top_k = gr.Slider(
            ...
            # with gr.Column():
            #     gr.Markdown(value=i18n("?‹å·¥è°ƒæ•´?³ç´ ?‚å½“?³ç´ æ¡†ä¸ä¸ºç©º?¶ä½¿?¨æ‰‹å·¥éŸ³ç´ è¾“?¥æ¨?†ï¼Œ? è§†?? ‡?‡æœ¬æ¡†ã€?))
            #     phoneme=gr.Textbox(label=i18n("?³ç´ æ¡?), value="")
            #     get_phoneme_button = gr.Button(i18n("?? ‡?‡æœ¬è½¬éŸ³ç´?), variant="primary")
        with gr.Row():
            inference_button = gr.Button(value=i18n("?ˆæˆè¯?Ÿ³"), variant="primary", size="lg", scale=25)
            output = gr.Audio(label=i18n("è¾“å‡º?„è???), scale=14)
        ...
        # gr.Markdown(value=i18n("?‡æœ¬?‡åˆ†å·¥å…·?‚å¤ª?¿çš„?‡æœ¬?ˆæˆ?ºæ¥?ˆæœä¸ä?å®šå?ï¼Œæ?ä»¥å¤ª?¿å»ºè®?…ˆ?‡ã€‚åˆ?ä¼š?¹æ®?‡æœ¬?„æ¢è¡Œåˆ†å¼€?ˆæˆ?æ‹¼èµ·æ¥??))
        # with gr.Row():
        #     text_inp = gr.Textbox(label=i18n("?€è¦åˆ?çš„?‡åˆ†?æ–‡??), value="")
        #     button1 = gr.Button(i18n("?‘å››?¥ä???), variant="primary")
        #     button2 = gr.Button(i18n("??0å­—ä???), variant="primary")
        #     button3 = gr.Button(i18n("?‰ä¸­?‡å¥?·ã€‚åˆ‡"), variant="primary")
        #     button4 = gr.Button(i18n("?‰è‹±?‡å¥????), variant="primary")
        #     button5 = gr.Button(i18n("?‰æ ‡?¹ç¬¦?·åˆ‡"), variant="primary")
        #     text_opt = gr.Textbox(label=i18n("?‡åˆ†?æ–‡??), value="")
        #     button1.click(cut1, [text_inp], [text_opt])
        #     button2.click(cut2, [text_inp], [text_opt])
        #     button3.click(cut3, [text_inp], [text_opt])
        #     button4.click(cut4, [text_inp], [text_opt])
        #     button5.click(cut5, [text_inp], [text_opt])
        # gr.Markdown(html_center(i18n("?ç»­å°†æ”¯?è½¬?³ç´ ?æ‰‹å·¥ä¿®?¹éŸ³ç´ ã€è??³åˆ?åˆ†æ­¥æ‰§è¡Œã€?)))
