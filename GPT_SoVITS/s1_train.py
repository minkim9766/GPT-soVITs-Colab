    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if self._should_save_on_train_epoch_end(trainer):
            ...
            if self._every_n_epochs >= 1 and (trainer.current_epoch + 1) % self._every_n_epochs == 0:
                ...
                ):  ####å¦‚æœè®¾ç½®?ªä¿å­˜æ??ä?ä¸ªckptï¼Œåœ¨ä¿å­˜ä¸‹ä?ä¸ªckpt?è¦æ¸…ç†?‰ä¹‹?çš„?€?‰ckpt
...
def main(args):
    ...
    trainer: Trainer = Trainer(
        ...
        # val_check_interval=9999999999999999999999,###ä¸è¦éªŒè¯
        ...
        use_distributed_sampler=False,  # ?å¸¸ç®€?•çš„ä¿?”¹ï¼Œä½†è§£å†³äº†é‡‡?¨è‡ªå®šä¹‰??bucket_sampler ä¸‹è?ç»ƒæ??°ä¸ä¸€?´çš„??¢˜ï¼?
    )
    ...
    try:
        # ä½¿ç”¨æ­£åˆ™è¡¨è¾¾å¼åŒ¹?æ–‡ä»¶åä¸?š„?°å­—?¨åˆ†ï¼Œå¹¶?‰æ•°å­—å¤§å°è¿›è¡Œæ’åº?
