...
class GPTSoVITSV3(torch.nn.Module):
    ...
    def forward(
        ...
        while 1:
            ...
            # ?�为导出?�模?�在不同shape?�会?�新编译还是?�么?�，会卡�?0s这样�?
            # ?�以在这里�?让他shape维持不变
            # 但是这样会�??�生?�的?�频?�度不�?，�?以在?�?�截?��?下�?
            # 经过 bigvgan 之后?�频?�度就是 fea_todo.shape[2] * 256
...
class GPTSoVITSV4(torch.nn.Module):
    ...
    def forward(
        ...
        while 1:
            ...
            # ?�以在这里�?让他shape维持不变
            # 但是这样会�??�生?�的?�频?�度不�?，�?以在?�?�截?��?下�?
            # 经过 hifigan 之后?�频?�度就是 fea_todo.shape[2] * 480
            complete_len = chunk_len - fea_todo_chunk.shape[-1]
            if complete_len != 0:
                fea_todo_chunk = torch.cat(
                    [
...
def get_sovits_weights(sovits_path):
    ...
    if if_lora_v3 == True and is_exist_s2gv3 == False:
        logger.info("SoVITS V3 底模缺失，无法加载相�?LoRA ?�重")
    ...
    logger.info(f"模型?�本: {model_version}")
...
def export_1(ref_wav_path, ref_wav_text, version="v3"):
    ...
    #     "你这?�坏?�，?�找了你这么久，?�没?�到?�这?�找?�你?�他说�?, "all_zh", "v3"
    # )
    phones1, bert1, norm_text1 = get_phones_and_bert(ref_wav_text, "auto", "v3")
    phones2, bert2, norm_text2 = get_phones_and_bert(
        "这是一个�??�的示例，真没想?�这么�??�就完成了。The King and His Stories.Once there was a king. He likes to write stories, but his stories were not good. As people were afraid of him, they all said his stories were good.After reading them, the writer at once turned to the soldiers and said: Take me back to prison, please.",
        "auto",
        "v3",
    )
...
def export_2(version="v3"):
    ...
    test_export(
        "汗流浃背了�?!?�弟~ My uncle has two dogs. One is big and the other is small. He likes them very much. He often plays with them. He takes them for a walk every day. He says they are his good friends. He is very happy with them. ?�?�还??��得了 MVP....",
        gpt_sovits_v3v4,
        "out.wav",
        sr,
    )
    ...
    test_export(
        "你小子是什么来�?汗流浃背了�?!?�弟~ My uncle has two dogs. He is very happy with them. ?�?�还??��得了 MVP!",
        gpt_sovits_v3v4,
        "out2.wav",
        sr,
    )
    ...
    # test_export(
    #     "汗流浃背了�?!?�弟~ My uncle has two dogs. One is big and the other is small. He likes them very much. He often plays with them. He takes them for a walk every day. He says they are his good friends. He is very happy with them. ?�?�还??��得了 MVP. ?�哈??..",
    #     gpt_sovits_v3_half,
    #     cfm,
    #     bigvgan_model,
    #     "out2.wav",
...
with torch.no_grad():
    # export_1("onnx/ad/ref.wav","你这?�坏?�，?�找了你这么久，?�没?�到?�这?�找?�你?�他说�?,"v4")
    export_2("v4")
