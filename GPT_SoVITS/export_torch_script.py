...
def export_bert(output_path):
    ...
    text = "?πÊÅØÂ£∞‰?Â£∞Êé•?Ä‰∏ÄÂ£∞‰º†???®ÂÖ∞ÂØπÁ??øÈó®ÁªáÂ∏É.?¨‰∏çËßÅÁªáÂ∏ÉÊú∫ÁªáÂ∏É?ÑÂ£∞???™Âê¨ËßÅÊú®?∞Âú®?πÊÅØ.??ú®?∞Âú®?≥‰?‰π???ú®?∞Âú®???‰ªÄ‰π??®ÂÖ∞Á≠îÈÅì,?ë‰πüÊ≤°Êúâ?®ÊÉ≥‰ªÄ‰π?‰πüÊ≤°?âÂú®???‰ªÄ‰π?"
...
def export(gpt_path, vits_path, ref_audio_path, ref_text, output_path, export_bert_and_ssl=False, device="cpu"):
    if not os.path.exists(output_path):
        ...
        print(f"??ΩïÂ∑≤ÂàõÂª? {output_path}")
    else:
        print(f"??ΩïÂ∑≤Â≠ò?? {output_path}")
    ...
    text_seq_id, text_bert_T, norm_text = get_phones_and_bert(
        "ËøôÊòØ‰∏Ä‰∏™Á??ïÁöÑÁ§∫‰æãÔºåÁúüÊ≤°ÊÉ≥?∞Ëøô‰πàÁ??ïÂ∞±ÂÆåÊàê‰∫Ü„ÄÇThe King and His Stories.Once there was a king. He likes to write stories, but his stories were not good. As people were afraid of him, they all said his stories were good.After reading them, the writer at once turned to the soldiers and said: Take me back to prison, please.",
        "auto",
        "v2",
    )
...
def export_prov2(
    ...
    top_k = torch.LongTensor([5]).to(device)
    # ?àË∑ë‰∏Ä??sv_model ËÆ©ÂÆÉ?†ËΩΩ cacheÔºåË??ÖËßÅ L880
...
class ExportERes2NetV2(nn.Module):
    ...
    def forward(self, audio_16k):
        # Ëøô‰∏™ fbank ?ΩÊï∞?â‰?‰∏?cache, ‰∏çËøá‰∏çË¶ÅÁ¥ßÔºåÂÆÉË∑ü audio_16k ?ÑÈïøÂ∫?ó†??
        # ?™Ë∑ü device ??dtype ?âÂÖ≥
...
def test():
    ...
    ref_bert = ref_bert_T.T.to(ref_seq.device)
    # text_seq_id,text_bert_T,norm_text = get_phones_and_bert("?®Â§©?ö‰∏ä?ãËßÅÂæÅÂÖµ?á‰π¶,?•ÈÅì?õ‰∏ª?®Â§ßËßÑÊ®°ÂæÅÂÖµ,?£‰πàÂ§öÂç∑ÂæÅÂÖµ?áÂÜå,ÊØè‰??∑‰∏ä?ΩÊúâ?∂‰∫≤?ÑÂêçÂ≠?","all_zh",'v2')
    text = "?®Â§©?ö‰∏ä?ãËßÅÂæÅÂÖµ?á‰π¶,?•ÈÅì?õ‰∏ª?®Â§ßËßÑÊ®°ÂæÅÂÖµ,?£‰πàÂ§öÂç∑ÂæÅÂÖµ?áÂÜå,ÊØè‰??∑‰∏ä?ΩÊúâ?∂‰∫≤?ÑÂêçÂ≠?"
