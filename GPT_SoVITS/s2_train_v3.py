...
torch.set_float32_matmul_precision("medium")  # ?�低精度但?�快（也就快�?丁点），对于结果?�成不了影响
# from config import pretrained_s2G,pretrained_s2D
global_step = 0

device = "cpu"  # cuda以外?��?备，等mps优化?�加??
...
def run(rank, n_gpus, hps):
    ...
    optim_g = torch.optim.AdamW(
        filter(lambda p: p.requires_grad, net_g.parameters()),  ###默�??�?�层lr一??
