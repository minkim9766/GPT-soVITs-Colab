...
class GPTSoVITSGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        license_text = (
            "?¬è½¯ä»¶ä»¥MIT?è?å¼€æº? ä½œè€…ä¸å¯¹è½¯ä»¶å…·å¤‡ä»»ä½•æ§?¶åŠ›, ä½¿ç”¨è½?»¶?…ã€ä¼ ??½¯ä»¶å??ºçš„å£°éŸ³?…è‡ªè´Ÿå…¨è´? "
            "å¦‚ä¸è®¤å¯è¯¥æ¡æ¬? ?™ä¸?½ä½¿?¨æˆ–å¼•ç”¨è½?»¶?…å†…ä»»ä½•ä»£ç ?Œæ–‡ä»? è¯?§?¹ç›®å½•LICENSE."
        )
        license_label = QLabel(license_text)
        license_label.setWordWrap(True)

        self.GPT_model_label = QLabel("?‰æ‹©GPTæ¨¡å‹:")
        self.GPT_model_input = QLineEdit()
        self.GPT_model_input.setPlaceholderText("?–æ‹½?–é€‰æ‹©?‡ä»¶")
        self.GPT_model_input.setText(self.GPT_Path)
        self.GPT_model_input.setReadOnly(True)
        self.GPT_model_button = QPushButton("?‰æ‹©GPTæ¨¡å‹?‡ä»¶")
        self.GPT_model_button.clicked.connect(self.select_GPT_model)

        self.SoVITS_model_label = QLabel("?‰æ‹©SoVITSæ¨¡å‹:")
        self.SoVITS_model_input = QLineEdit()
        self.SoVITS_model_input.setPlaceholderText("?–æ‹½?–é€‰æ‹©?‡ä»¶")
        self.SoVITS_model_input.setText(self.SoVITS_Path)
        self.SoVITS_model_input.setReadOnly(True)
        self.SoVITS_model_button = QPushButton("?‰æ‹©SoVITSæ¨¡å‹?‡ä»¶")
        self.SoVITS_model_button.clicked.connect(self.select_SoVITS_model)

        self.ref_audio_label = QLabel("ä¸Šä¼ ?‚è€ƒéŸ³é¢?")
        self.ref_audio_input = QLineEdit()
        self.ref_audio_input.setPlaceholderText("?–æ‹½?–é€‰æ‹©?‡ä»¶")
        self.ref_audio_input.setReadOnly(True)
        self.ref_audio_button = QPushButton("?‰æ‹©?³é¢‘?‡ä»¶")
        self.ref_audio_button.clicked.connect(self.select_ref_audio)

        self.ref_text_label = QLabel("?‚è€ƒéŸ³é¢‘æ–‡??")
        self.ref_text_input = QLineEdit()
        self.ref_text_input.setPlaceholderText("?´æ¥è¾“å…¥?‡å­—?–ä¸Šä¼ æ–‡??)
        self.ref_text_button = QPushButton("ä¸Šä¼ ?‡æœ¬")
        self.ref_text_button.clicked.connect(self.upload_ref_text)

        self.ref_language_label = QLabel("?‚è€ƒéŸ³é¢‘è?è¨€:")
        self.ref_language_combobox = QComboBox()
        self.ref_language_combobox.addItems(["ä¸?–‡", "?±æ–‡", "?¥æ–‡", "ä¸?‹±æ··åˆ", "?¥è‹±æ··åˆ", "å¤šè?ç§æ··??])
        self.ref_language_combobox.setCurrentText("å¤šè?ç§æ··??)

        self.target_text_label = QLabel("?ˆæˆ?? ‡?‡æœ¬:")
        self.target_text_input = QLineEdit()
        self.target_text_input.setPlaceholderText("?´æ¥è¾“å…¥?‡å­—?–ä¸Šä¼ æ–‡??)
        self.target_text_button = QPushButton("ä¸Šä¼ ?‡æœ¬")
        self.target_text_button.clicked.connect(self.upload_target_text)

        self.target_language_label = QLabel("?ˆæˆ?³é¢‘è¯??:")
        self.target_language_combobox = QComboBox()
        self.target_language_combobox.addItems(["ä¸?–‡", "?±æ–‡", "?¥æ–‡", "ä¸?‹±æ··åˆ", "?¥è‹±æ··åˆ", "å¤šè?ç§æ··??])
        self.target_language_combobox.setCurrentText("å¤šè?ç§æ··??)

        self.output_label = QLabel("è¾“å‡º?³é¢‘è·?¾„:")
        self.output_input = QLineEdit()
        self.output_input.setPlaceholderText("?–æ‹½?–é€‰æ‹©?‡ä»¶")
        self.output_input.setReadOnly(True)
        self.output_button = QPushButton("?‰æ‹©?‡ä»¶å¤?)
        ...
        self.synthesize_button = QPushButton("?ˆæˆ")
        self.synthesize_button.clicked.connect(self.synthesize)

        self.clear_output_button = QPushButton("æ¸…ç©ºè¾“å‡º")
    ...
    def select_GPT_model(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "?‰æ‹©GPTæ¨¡å‹?‡ä»¶", "", "GPT Files (*.ckpt)")
        if file_path:
            self.GPT_model_input.setText(file_path)

    def select_SoVITS_model(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "?‰æ‹©SoVITSæ¨¡å‹?‡ä»¶", "", "SoVITS Files (*.pth)")
        if file_path:
            self.SoVITS_model_input.setText(file_path)

    def select_ref_audio(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "?‰æ‹©?‚è€ƒéŸ³é¢‘æ–‡ä»?, "", "Audio Files (*.wav *.mp3)")
        if file_path:
            self.update_ref_audio(file_path)

    def upload_ref_text(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "?‰æ‹©?‡æœ¬?‡ä»¶", "", "Text Files (*.txt)")
        if file_path:
    ...
    def synthesize(self):
        ...
        self.status_bar.showMessage("?ˆæˆå®Œæˆï¼è¾“?ºè·¯å¾„ï¼š" + output_wav_path, 5000)
        self.output_text.append("å¤„ç†ç»“æœï¼?n" + result)
