...
def split_jako(tag_lang,item):
    if tag_lang == "ja":
        pattern = r"([\u3041-\u3096\u3099\u309A\u30A1-\u30FA\u30FC]+(?:[0-9???œã€‚ï¼ï¼?!???]+[\u3041-\u3096\u3099\u309A\u30A1-\u30FA\u30FC]*)*)"
    else:
        pattern = r"([\u1100-\u11FF\u3130-\u318F\uAC00-\uD7AF]+(?:[0-9???œã€‚ï¼ï¼?!???]+[\u1100-\u11FF\u3130-\u318F\uAC00-\uD7AF]*)*)"
                ...
                if temp_item['lang'] == 'digit':
                    if default_lang:
                        ...
                        if lang_list[-1]['lang'] == temp_list[i + 1]['lang']:
                            temp_item['lang'] = lang_list[-1]['lang']
                        elif lang_list[-1]['text'][-1] in [",",".","!","?","ï¼?,"??,"ï¼?,"ï¼?]:
                            temp_item['lang'] = temp_list[i + 1]['lang']
                        elif temp_list[i + 1]['text'][0] in [",",".","!","?","ï¼?,"??,"ï¼?,"ï¼?]:
                            temp_item['lang'] = lang_list[-1]['lang']
...
if __name__ == "__main__":
    text = "MyGO?,ä½ ä¹Ÿ?œæ¬¢?¾ã„?”å—ï¼?
    print(LangSegmenter.getTexts(text))

    text = "??ˆ?çŸ¥?£ã¦?‹ï¼Ÿ?€è¿‘ã€åƒ•??¤©?‡å??’å‹‰å¼·ã—?¦ã‚‹?“ã ?‚å›??³?Œæ˜Ÿç©ºã¿?Ÿã„?«ã‚­?©ã‚­?©ã—?¦ã‚‹?‹ã‚‰?•ã€?
    print(LangSegmenter.getTexts(text))

    text = "å½“æ—¶ThinkPad T60?šåˆš?‘å¸ƒï¼Œä??Œæ¨?ºçš„è¿˜æœ‰ä¸€æ¬¾åä¸ºAdvanced Dock?„æ‰©å±•å?ä»¶?‚è¿™æ¬¾æ‰©å±•å?šè¿‡è¿æ¥T60åº•éƒ¨?„æ’æ§½ï¼Œ?©å±•?ºåŒ…?¬PCIe?¨å†…?„ä?å¤§å †?¥å£ï¼Œå¹¶ä¸”è‡ªå¸?”µæºï¼Œè®©T60??»¥å®‰è£…æ¡Œé¢?¾å¡?¥æ?‡æ€§èƒ½??
