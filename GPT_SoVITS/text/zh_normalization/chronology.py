...
def replace_time(match) -> str:
    ...
    if second and second.lstrip("0"):
        result += f"{_time_num2str(second)}ç§?
    ...
    if is_range:
        ...
        if second_2 and second_2.lstrip("0"):
            result += f"{_time_num2str(second_2)}ç§?
...
def replace_date(match) -> str:
    ...
    result = ""
    if year:
        result += f"{verbalize_digit(year)}å¹?
    if month:
