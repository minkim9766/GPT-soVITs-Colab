...
def text_normalize(text):
    ...
    # ?ˆæœ?¸åŒï¼Œå’Œ chinese.py ä¿æŒä¸€??
...
class en_G2p(G2p):
    ...
    def __call__(self, text):
        ...
        for o_word, pos in tokens:
            ...
            # ?ˆæŠŠ?•å­—æ¯æ¨?ºå»
                ...
                # ?•è? A ?‘éŸ³ä¿??, è¿™é‡Œ?€è¦åŸ?¼å¼ o_word ?¤æ–­å¤§å†™
                if o_word == "A":
                    pron = ["EY1"]
                else:
                    pron = self.cmu[word][0]
            # g2p_en ?Ÿç‰ˆå¤šéŸ³å­—å¤„??
    ...
    def qryword(self, o_word):
        ...
        # ?•è¯ä»…é¦–å­—æ¯å¤§å†™?¶æŸ¥?¾å§“?å­—??
        if o_word.istitle() and word in self.namedict:
            return self.namedict[word][0]

        # oov ?¿åº¦å°äºç­‰äº 3 ?´æ¥è¯»å­—æ¯?
        if len(word) <= 3:
            phones = []
            for w in word:
                # ?•è? A ?‘éŸ³ä¿??, æ­¤å¤„ä¸å­˜?¨å¤§?™çš„?…å†µ
                if w == "a":
                    phones.extend(["EY1"])
                elif not w.isalpha():
                    phones.extend([w])
        ...
        if re.match(r"^([a-z]+)('s)$", word):
            ...
            # P T K F TH HH ? å£°è¾…éŸ³ç»“å°¾ 's ??['S']
            if phones[-1] in ["P", "T", "K", "F", "TH", "HH"]:
                phones.extend(["S"])
            # S Z SH ZH CH JH ??£°ç»“å°¾ 's ??['IH1', 'Z'] ??['AH0', 'Z']
            elif phones[-1] in ["S", "Z", "SH", "ZH", "CH", "JH"]:
                phones.extend(["AH0", "Z"])
            # B D G DH V M N NG L R W Y ?‰å£°è¾…éŸ³ç»“å°¾ 's ??['Z']
            # AH0 AH1 AH2 EY0 EY1 EY2 AE0 AE1 AE2 EH0 EH1 EH2 OW0 OW1 OW2 UH0 UH1 UH2 IY0 IY1 IY2 AA0 AA1 AA2 AO0 AO1 AO2
            # ER ER0 ER1 ER2 UW0 UW1 UW2 AY0 AY1 AY2 AW0 AW1 AW2 OY0 OY1 OY2 IH IH0 IH1 IH2 ?ƒéŸ³ç»“å°¾ 's ??['Z']
            else:
                phones.extend(["Z"])
            return phones

        # å°è¯•è¿›è¡Œ?†è¯ï¼Œåº”å¯¹å¤?ˆè¯
...
def g2p(text):
    # g2p_en ?´æ??¨ç†ï¼Œå‰”?¤ä¸å­˜åœ¨?„arpaè¿”å›
