...
try:
    ...
    # Â¶ÇÊûúÊ≤°Êúâ?®Êà∑ËØçÂÖ∏ÔºåÂ∞±?üÊàê‰∏Ä‰∏™ÔºõÂ¶ÇÊûú?âÔºåÂ∞±Ê??•md5ÔºåÂ¶Ç?ú‰∏ç‰∏Ä?∑ÔºåÂ∞±Èáç?∞Áîü??
...
_symbols_to_japanese = [(re.compile("%s" % x[0]), x[1]) for x in [("Ôº?, "?ë„Éº?ª„É≥??)]]
...
def pyopenjtalk_g2p_prosody(text, drop_unvoiced_vowels=True):
    ...
    Examples:
        ...
        >>> pyopenjtalk_g2p_prosody("?ì„Çì?´„Å°??Ä?)
        ['^', 'k', 'o', '[', 'N', 'n', 'i', 'ch', 'i', 'w', 'a', '$']
...
if __name__ == "__main__":
    phones = g2p("Hello.?ì„Çì?´„Å°??ºÅ‰ªäÊó•?ÇNiCeÂ§©Ê∞ó?ß„Åô??ºÅtokyotower?´Ë°å?ç„Åæ?ó„Çá?ÜÔºÅ")
    print(phones)
