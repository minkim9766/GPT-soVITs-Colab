# 这是一个实验性质?�实?�，?�在?�索 stream infer ?�可?�性�?xiao hai xie zhe wan de)
...
class StreamT2SModel(nn.Module):
    ...
    def pre_infer(
        ...
        x_attn_mask_pad = F.pad(
            x_attn_mask,
            (0, y_len),  ###xx?�纯0?�展?�xx�?+xy�?�?x,x+y)
            value=True,
        )
        y_attn_mask = F.pad(  ###yy?�右�??�展?�左边xy??,(y,x+y)
...
def find_best_audio_offset_fast(reference_audio: Tensor, search_audio: Tensor):
    ...
    if search_len < ref_len:
        raise ValueError(
            f"?�索?�频?�度 ({search_len}) 必须大于等于?�考音频长�?({ref_len})"
        )
...
def test_stream(
    ...
    text_seq_id, text_bert_T, norm_text = get_phones_and_bert(
        "这是一个�??�的示例，真没想?�这么�??�就完成了，?�的神奇，接下来?�们说�??�狸,??��这就??��?�吧.它有?�长?�尾巴，尖尖?�耳朵，传说中还有九条尾巴?�你觉得?�狸神奇?�？", "auto", "v2"
    )
...
def export_prov2(
    ...
    text_seq_id, text_bert_T, norm_text = get_phones_and_bert(
        "这是一个�??�的示例，真没想?�这么�??�就完成�?The King and His Stories.Once there was a king.He likes to write stories, but his stories were not good.", "auto", "v2"
    )
    text_seq = torch.LongTensor([text_seq_id]).to(device)
    text_bert = text_bert_T.T
    if is_half:
